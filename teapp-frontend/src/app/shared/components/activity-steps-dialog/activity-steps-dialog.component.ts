...
interface StepDraft {
  ...
  arasaacQuery: string;
  arasaacResults: { _id: number; keyword: string }[];
  arasaacSearching: boolean;
  imageMode: 'upload' | 'arasaac';
}
...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content class="steps-content">
      <div class="steps-list" *ngIf="!loading">
        ...
        <div class="step-card" *ngFor="let step of steps; let i = index">
          ...
          <div class="step-body">
            ...
            <div class="image-mode-tabs">
              <button type="button" class="img-mode-btn" [class.active]="step.imageMode === 'upload'"
                      (click)="step.imageMode = 'upload'">
                <mat-icon>upload</mat-icon> Subir
              </button>
              <button type="button" class="img-mode-btn" [class.active]="step.imageMode === 'arasaac'"
                      (click)="step.imageMode = 'arasaac'">
                <mat-icon>image_search</mat-icon> ARASAAC
              </button>
            </div>

            <!-- Upload -->
            <div *ngIf="step.imageMode === 'upload'">
            ...
            <!-- ARASAAC -->
            <div *ngIf="step.imageMode === 'arasaac'" class="arasaac-mini">
              <div class="arasaac-search-row">
                <input class="arasaac-input" [(ngModel)]="step.arasaacQuery"
                       placeholder="Buscar pictograma..." (keyup.enter)="searchArasaac(step)">
                <button mat-icon-button (click)="searchArasaac(step)" [disabled]="step.arasaacSearching">
                  <mat-spinner *ngIf="step.arasaacSearching" diameter="16"></mat-spinner>
                  <mat-icon *ngIf="!step.arasaacSearching">search</mat-icon>
                </button>
              </div>
              <div class="arasaac-grid-mini" *ngIf="step.arasaacResults.length > 0">
                <button type="button" class="arasaac-thumb"
                        *ngFor="let pic of step.arasaacResults"
    ...
    <mat-dialog-actions align="end">
      ...
      <button mat-flat-button class="btn-save" (click)="save()" [disabled]="saving || steps.length === 0">
...
export class ActivityStepsDialogComponent implements OnInit {
    ...
    this.activityService.getSteps(this.data.activity.id).subscribe({
      next: (steps) => {
        this.steps = steps.map(s => ({
          ...
          arasaacQuery: '', arasaacResults: [], arasaacSearching: false,
          imageMode: s.pictogramUrl ? 'arasaac' : 'upload'
        }));
        this.loading = false;
      },
  ...
  addStep(): void {
    this.steps.push({
      ...
      arasaacQuery: '', arasaacResults: [], arasaacSearching: false, imageMode: 'upload'
    });
  }
  ...
  searchArasaac(step: StepDraft): void {
    if (!step.arasaacQuery.trim()) return;
    step.arasaacSearching = true;
    step.arasaacResults = [];
    this.arasaacService.search(step.arasaacQuery.trim()).subscribe({
      next: (r) => { step.arasaacResults = r; step.arasaacSearching = false; },
      error: () => { step.arasaacSearching = false; }
    });
  }
  ...
  save(): void {
    ...
    this.activityService.saveSteps(this.data.activity.id, requests).subscribe({
      ...
      error: () => { this.snackBar.open('Error al guardar', 'Cerrar', { duration: 3000 }); this.saving = false; }
    });
  }
}
