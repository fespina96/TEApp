...
export class TimeSlotColumnComponent implements OnChanges {
  ...
  openActivityPicker(): void {
    ...
    dialogRef.afterClosed().subscribe((result?: {
      activity: Activity; addToAllDays: boolean;
      durationMinutes?: number; pausable?: boolean; requireFullTimer?: boolean;
    }) => {
      if (!result?.activity) return;

      const { activity, addToAllDays, durationMinutes, pausable, requireFullTimer } = result;
      const days = addToAllDays ? DAYS_OF_WEEK : [this.day];
      ...
      forkJoin(requests).subscribe({
        next: () => {
          const msg = addToAllDays ? 'Actividad agregada a todos los días' : 'Actividad agregada';
